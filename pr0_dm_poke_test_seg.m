...
load /proj/jwst2/jzlou/git-6MST/old-6MST-jzlouStuff/macos/opd_wfc_4Erkin/segMask.mat
%whos, return
%pupil0 = fitsread('/home/esidick/2022habex/dat_macos/pup256_macos.fits');
pupil0 = fitsread('/home/esidick/Afalco/falco20200916/macos/maps_20230530/pupil_gray_gap08.fits');
...
dval = 0.2; % [nm], piston/tip/tilt

ns = 19;

%video = VideoWriter('dat_img/my_movie_segment_tip.avi'); %create the video object
video = VideoWriter('dat_img/erkin_movie_segment_tilt.avi'); %create the video object
%video = VideoWriter('dat_img/my_movie_segment_tilt.avi'); %create the video object
video.FrameRate = 1;
open(video); %open the file for writing

for ii = 1:19

    maskz = double(segM(1,ii).mask);
    opdz = zernike_compose(maskz, [0 0 dval]);

    pupil = pupil0 .* exp(1i*2*pi*(opdnm+opdz)*1e-9/mp.lambda0);
