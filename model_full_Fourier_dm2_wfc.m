...
function [Eout, varargout] = model_full_Fourier_dm2_wfc(mp, lambda, Ein, normFac)
...
EP3 = propcustom_relay(EP2eff,mp.Nrelay2to3,mp.centering);
    ...
    %pha_nm = real(pha_nm);
    opd = (pha_nm - mp.wfc.opd_target)  .* pad(mp.P1.full.mask,np); %mp.wfc.maskopd;
    amp = (abs(ce) - mp.wfc.amp_target); %mp.wfc.maskopd;
