...
mp.fracBW  = 0.05; %0.10;       %--fractional bandwidth of the whole bandpass (Delta lambda / lambda0)
mp.Nsbp    = 3; %5;            %--Number of sub-bandpasses to divide the whole bandpass into for estimation and control
...
mp.dm_ind = [1]; %[1 2]; %--Which DMs to use
...
mp.dm1.dm_spacing = 0.9906e-3; %--User defined actuator pitch [meters]
mp.dm2.dm_spacing = 0.9906e-3; %--User defined actuator pitch [meters]
...
%--DM1 parameters
m0 = 48;
mp.dm1.Nact = m0;               % # of actuators across DM array
mp.dm1.VtoH = 1*1e-9*ones(m0);  % gains of all actuators [nm/V of free stroke]
mp.dm1.xtilt = 0;               % for foreshortening. angle of rotation about x-axis [degrees]
mp.dm1.ytilt = 9.65;               % for foreshortening. angle of rotation about y-axis [degrees]
mp.dm1.zrot = 0;                % clocking of DM surface [degrees]
mp.dm1.xc = (m0/2 - 1/2);       % x-center location of DM surface [actuator widths]
mp.dm1.yc = (m0/2 - 1/2);       % y-center location of DM surface [actuator widths]
mp.dm1.edgeBuffer = 1;          % max radius (in actuator spacings) outside of beam on DM surface to compute influence functions for. [actuator widths]

%--DM2 parameters
mp.dm2.Nact = m0;               % # of actuators across DM array
mp.dm2.VtoH = 1*1e-9*ones(m0);  % gains of all actuators [nm/V of free stroke]
...
mp.dm2.xc = (m0/2 - 1/2);       % x-center location of DM surface [actuator widths]
mp.dm2.yc = (m0/2 - 1/2);       % y-center location of DM surface [actuator widths]
...
mp.flagDM2stop = false; %true;  %--Whether to apply an iris or not
mp.dm2.Dstop   = diam; %0.4*34e-3;   %--Diameter of iris [meters]
%mp.dm2.Dstop = 0.4*34e-3;   %--Diameter of iris [meters]

%--DM separations
mp.d_P2_dm1 = 0;        % distance (along +z axis) from P2 pupil to DM1 [meters]
mp.d_dm1_dm2 = 1.0;   % distance between DM1 and DM2 [meters]
...
mp.Fend.corr.Rin = 1.0;   % inner radius of dark hole correction region [lambda0/D]
mp.Fend.corr.Rout  = 10;  % outer radius of dark hole correction region [lambda0/D]
mp.Fend.corr.ang  = 180;  % angular opening of dark hole correction region [degrees]

mp.Fend.score.Rin = 1.0;  % inner radius of dark hole scoring region [lambda0/D]
...
mp.P2.D = diam; %0.4*32e-3;
mp.P3.D = diam; %0.4*32e-3;
mp.P4.D = diam; %0.4*32e-3;
%mp.P2.D = 0.4*32e-3;
%mp.P3.D = 0.4*32e-3;
%mp.P4.D = 0.4*32e-3;
...
m0 = 1024;
mp.P1.compact.Nbeam = m0;
mp.P2.compact.Nbeam = m0;
mp.P3.compact.Nbeam = m0;
mp.P4.compact.Nbeam = m0;  % P4 must be the same as P1 for Vortex. 
...
mp.P2.full.Nbeam = m0;
mp.P3.full.Nbeam = m0;
mp.P4.full.Nbeam = m0;  % P4 must be the same as P1 for Vortex. 
...
%--Lyot stop padding
mp.P4.IDnorm = 0.25; %--Lyot stop ID [Dtelescope]
