...
xv = [-nr/2:nr/2-1] / mp.Fend.res;
    ...
    figure(1), clf
        ...
        title(['825nm-NI: $\delta$ = 0,' sprintf('Cb = %0.2g', cb)],'Fontsize',fs,'Interpreter', 'Latex');
   print -dpng ../Figs/fig_psf1a
    ...
    figure(2), clf, semilogy(rv, psfv, 'ro-'), grid
        ...
        title('825nm: Radial Average of 10\% Broadband NI','Fontsize',fs,'Interpreter', 'Latex');
    print -dtiff ../Figs/fig_psfv1a
...
if 0    
    ...
    aa = dm1; rmsa = stat2d(aa); cx = [min(aa(:)) max(aa(:))];
...
%end
    ...
    figure(2), 
        xlabel('Radial Field-Angle [$\lambda$/D]', 'Fontsize',fs,'Interpreter', 'Latex');
        ylabel('Radial Average of NI', 'Fontsize',fs,'Interpreter', 'Latex');
        title('512pix: Pre-EFC Radial Average of 10\% Broadband NI','Fontsize',fs,'Interpreter', 'Latex');
        legend(legx, 'Location', 'Southeast')
        axis([0 23 1e-11 1e-5])
    %print -dtiff ../Figs/fig_psfv
   
 %save ../dat_erkin/dat_apod512_psfx psfvx legx xx cbb cb0 psf0 xv  
