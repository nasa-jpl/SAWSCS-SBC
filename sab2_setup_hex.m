...
%mp.lambda0 = 1064e-9; %355e-9; %500e-9;    %--Central wavelength of the whole spectral bandpass [meters]
if exist('model_mode')
  switch model_mode
    case 'wfc' % Out-of-band
      mp.lambda0 = 1500e-9; %--Central wavelength of the whole spectral bandpass [meters]
    ...
    case 'eval'
      ...
      mp.Nsbp = 12;         %--Number of sub-bandpasses to divide the whole bandpass into for estimation and control
  end
else
  error('model_mode not defined')
end
...
mp.est.probe.radius = 25; %12;    % Max x/y extent of probed region [actuators].
...
%--Zernikes to compute sensitivities for
mp.eval.indsZnoll = [1 2 3]; %--Noll indices of Zernikes to compute values for
%--Annuli to compute 1nm RMS Zernike sensitivities over. Columns are [inner radius, outer radius]. One row per annulus.
%mp.eval.Rsens = [2,3; 3,4; 4,5];
mp.eval.Rsens = [1,3];
...
mp.ctrl.log10regVec = -5:1:-1;          %--log10 of the regularization exponents (often called Beta values)
...
mp.Fend.FOV = 15; %12; %11; %30; %--half-width of the field of view in both dimensions [lambda0/D]

%--Correction and scoring region definition
mp.Fend.corr.Rin  = 2; %2.0;   % inner radius of dark hole correction region [lambda0/D]
mp.Fend.corr.Rout = 12; %26;  % outer radius of dark hole correction region [lambda0/D]
...
mp.P1.compact.Nbeam = 256; %512;         % <<<<<<<<<<<<<<, pupil size <<<<<<
...
% mp.F3.full.res = 6;    % mp of FPM for full model [pixels per lambda0/D]
...
mp.P1.D = 6; %7.989; %--meters, circumscribed. The segment size is 0.955 m, flat-to-flat, and the gaps are 6 mm. %--telescope diameter [meters]. Used only for converting milliarcseconds to lambda0/D or vice-versa.
...
mp.apodType = 'Simple';
mp.P3.IDnorm = 0.2; %0;
mp.P3.ODnorm = 0.98; %0.84;
mp.full.flagGenApod = true;
mp.compact.flagGenApod = true;

%--Lyot stop padding
mp.P4.IDnorm = 0.25; %0; %0; %--Lyot stop ID [Dtelescope]
mp.P4.ODnorm = 0.96; %0.82; %--Lyot stop OD [Dtelescope]
mp.P4.padFacPct = 0;
